<div class="card shadow-sm"><div class="card-body">
    ...
    <form method="post" action="<?= e(url('/admin/contents/update')) ?>" enctype="multipart/form-data" class="row g-3">
        ...
        <div class="col-md-3"><select name="training_path" class="form-select" required><option value="">Formação (Know-how)</option><?php foreach (($knowledgePathOptions ?? []) as $path): ?><option value="<?= e($path) ?>"<?= ($content['training_path'] ?? '')===$path?' selected':'' ?>><?= e($path) ?></option><?php endforeach; ?></select></div>
        ...
        <div class="col-md-4"><input name="video_url" class="form-control" value="<?= e($content['video_url'] ?? '') ?>" placeholder="URL vídeo (opcional)"></div>
